...
int main(int argc, char* argv[]) {
    if (argc != 11) {
        printfln("usage: %s <R> <N> <l_max> <sweep_table> <dH2_goal> <sweeps_min> <sweeps_max> <sites_path> <H_path> <mps_path>", argv[0]);
    ...
    std::string sites_path = argv[8];
    std::string H_path = argv[9];
    std::string mps_path = argv[10];
    ...
    writeToFile(sites_path, sites);
    writeToFile(H_path, H);
    writeToFile(mps_path, psi);
